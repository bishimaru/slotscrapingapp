...
def get_data_BBstationNippori(day):
    with transaction.atomic():
        # TOPページから全機種名一覧を取得
        base_url = 'https://p-ken.jp/p-bbnippori/bonus/'
        response = requests.get(base_url)
        soup = BeautifulSoup(response.content, 'html.parser')
        titles_ul = soup.find("ul", class_="item_list")
        titles_li = titles_ul.find_all('li', class_="item_list")

        title_list = {}
        for title in titles_li:
            slot_name = title.find('div', class_="meter").contents[1].text
            slot_name_url = title.find("a").get('href')
            title_list[slot_name] = slot_name_url
        ...
        for name, href in title_list.items():
            data = []
            print('機種名/' + name)
            # 日付指定
            ...
            if day:
                encode_slot_title = re.search(r'(model_nm=(.+)&cost=21.7391&ps_div=2&mode=)', href).groups()
                print(encode_slot_title[1])
                # encode_slot_title = urllib.parse.quote(name, encoding='shift-jis')
                url = base_url + "lot?ps_div=2&cost=21.7391&model_nm=" + str(encode_slot_title[1]) + "&day=" + str(day) + "&mode="
            ...
            data2 = []
            data_list_on = wait.until(EC.element_to_be_clickable((By.TAG_NAME, 'button')))
            ...
            Store = PachiSlotStore.objects.get(pk=5)
            date = data[0]['date']
            date_format = "%Y-%m-%d"
